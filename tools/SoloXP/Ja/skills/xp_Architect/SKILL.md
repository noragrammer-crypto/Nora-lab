...
参謀・アーキテクトとして、イシューを分類しサブイシューを全件発行し、xp_Director へ実行計画を返す。
...
**`estimate.breakdown` が存在しない場合:**
- `xp_plan` を先に実行するよう xp_Director に報告して終了する
...
これにより ProcessIssue の `gh issue view <親番号> --json subIssues` が正しくサブイシューを返すようになる。
