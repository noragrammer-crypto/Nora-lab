...
> **現状（2026-08時点）**: 現行の運用フロー（GitHub Issue起点、`SoloXP/WORKFLOW.md` 参照）では
> `xp_Architect` からの自動呼び出しは行われない。もともとMarkdownのストーリーカードでイテレーション
> 計画を管理していた旧運用の名残であり、現在はイシューをGitHub上に置きラベルによるトリアージで
> イテレーション計画を実行する運用に切り替わっている（詳細: #2965）。StoryCardベースで手動にタスク
> 分解・見積もりをしたい場合の実行手段としてのみ残す。
