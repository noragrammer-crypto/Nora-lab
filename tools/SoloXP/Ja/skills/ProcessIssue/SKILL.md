...
イシュー選択・振り分け判定はいずれもルールベースの機械的処理であるため軽量モデル（Haiku）で動作する。
実質的な設計・実装判断は委譲先（`xp_Director`・`NovelGeneratorRun`・`ProcessCodexIssue`）が自身の
frontmatterで宣言するモデルで行われ、ProcessIssue自身のモデル変更の影響を受けない（#2907）。
...
**処理：**
xp_Director を呼ばず、`ProcessCodexIssue` に委譲する（判断・修正・PR作成・ignoreラベル付与は同スキルの責務。#2907 で分離）：
```
/ProcessCodexIssue <issue番号>
```

**連続する Codex イシューの処理：**
選択したイシューが Codex イシューだった場合、`/ProcessCodexIssue` の処理完了後、
2章の選択ロジックで次の候補イシューを再評価する。それも Codex イシューであれば続けて
`/ProcessCodexIssue` に委譲する（1セッションで連続処理可）。ただし通常のソフトウェア開発イシューが
混在している場合は1件で停止してユーザーに確認する。
...
- Codex イシュー（ワークフロー4）は `ProcessCodexIssue` への委譲後も連続処理可
- 判断に迷う場合はユーザーに確認する
